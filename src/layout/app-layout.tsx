...
import { getPresses } from 'apiSdk/presses';
...
const SidebarContent = ({ onClose, ...rest }: SidebarProps) => {
  ...
  const { data, error, isLoading } = useSWR<PressInterface[]>(
    () => (isTenantUser && session?.user?.tenantId ? `/presses` : null),
    () => getPresses({ tenant_id: session?.user?.tenantId }),
  );
  loading = isLoading;
  ...
  const LinkItems: Array<LinkItemProps> = [
    ...
    {
      name: 'Press',
      path: isTenantUser ? `/presses/view/${data?.[0]?.id}` : '/presses',
      entity: 'press',
    ...
    { name: 'Despatch', path: '/despatches', entity: 'despatch', service: AccessServiceEnum.PROJECT, icon: FiTruck },
    { name: 'Enquiry', path: '/enquiries', entity: 'enquiry', service: AccessServiceEnum.PROJECT, icon: FiHelpCircle },
