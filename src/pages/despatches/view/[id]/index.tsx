...
import { getDespatchById } from 'apiSdk/despatches';
...
function DespatchViewPage() {
  const { hasAccess } = useAuthorizationApi();
  const router = useRouter();
  const id = router.query.id as string;
  const { data, error, isLoading, mutate } = useSWR<DespatchInterface>(
    () => (id ? `/despatches/${id}` : null),
    () =>
      getDespatchById(id, {
      ...
      <Box bg="white" p={4} rounded="md" shadow="md">
        <Flex justifyContent="space-between" mb={4}>
          <Text as="h1" fontSize="2xl" fontWeight="bold">
            Despatch Detail View
          </Text>
          {hasAccess('despatch', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
            <NextLink href={`/despatches/edit/${data?.id}`} passHref legacyBehavior>
              <Button
